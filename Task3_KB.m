...
[Corr start_pos]= max(delay);% P=6288  find the start of the Pilot

PilotSignal=SampledSignal(start_pos:start_pos+Nsc-1);% the pilot samples N=128 symbols, if earlier ok
ofdm_first_cypr=SampledSignal(start_pos+Nsc:end); % the first cyclic prefix of the signal
...
%OFDM_blocks =floor(length(SampledSignal(start_pos-Ncp:end))/(Nsc+Ncp)); 
...
for i=1: OFDM_blocks 
   ...
   OFDM_sig = start_pos+(Nsc+Ncp)*i:start_pos+(Nsc+Ncp)*i+Nsc-1;
   cp = SampledSignal(OFDM_sig);
   fourier_signal(i,:) = fft(cp);
   
end
fourier_pil=fft(PilotSignal); 
